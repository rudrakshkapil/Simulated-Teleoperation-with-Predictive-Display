...
namespace ORB_SLAM2
{
    ...
    {
        //received_M=(cv::Mat_<float>(3,1) << 0,0,0);
        received_M = {0,0,0};
        received_R={1,0,0,0,1,0,0,0,1};
    }
...
//            static unsigned int frameTex[4] = {0, 0, 0, 0};
            ...
            for (list<dlovi::Matrix>::const_iterator it = GetTris().begin(); it != GetTris().end(); it++) {
                ...
                vector<int> indexTex;
                for (int i = 0; i < numKFs; i++){
                    //imAndTexFrame[i].second.mtcw = (cv::Mat_<float>(3, 1) << received_M[0], received_M[1], received_M[2]);
                    cv::Mat texOrient = imAndTexFrame[i].second.GetOrientation();
                    //std::cout << imAndTexFrame[i].second.mRcw << "\n" << imAndTexFrame[i].second.mtcw << "\n" << imAndTexFrame[i].second.mTwc << "\n\n\n"; // $$
                    //std::cout<< "MODEL DRAWER: " << received_M[0]<<" "<<received_M[1]<<" "<<received_M[2]<<"\n";
...
//                     //tex.mtcw = (cv::Mat_<float>(3, 1) << received_M[0], received_M[1], received_M[2]);
//                     vector<float> uv0 = tex.GetTexCoordinate(point0(0)-received_M[0],point0(1)-received_M[1],point0(2)-received_M[2],imSize);
//                     //std::cout<< "Point 0"<<
//                     vector<float> uv1 = tex.GetTexCoordinate(point1(0)-received_M[0],point1(1)-received_M[1],point1(2)-received_M[2],imSize);
//                     vector<float> uv2 = tex.GetTexCoordinate(point2(0)-received_M[0],point2(1)-received_M[1],point2(2)-received_M[2],imSize);
...
//                         glVertex3d(point0(0)-received_M[0],point0(1)-received_M[1],point0(2)-received_M[2]);

//                         glTexCoord2f(uv1[0], uv1[1]);
//                         glVertex3d(point1(0)-received_M[0],point1(1)-received_M[1],point1(2)-received_M[2]);

//                         glTexCoord2f(uv2[0], uv2[1]);
//                         glVertex3d(point2(0)-received_M[0],point2(1)-received_M[1],point2(2)-received_M[2]);
                     TextureFrame tex = imAndTexFrame[indexCurr].second;
                     std::cout<<"Old Translation value:"<<tex.mtcw<<"\n";
                     tex.mtcw = (cv::Mat_<float>(3, 1) << received_M[0]/10, received_M[1]/10, received_M[2]/10);
                     std::cout<<"New Translation Value:"<<tex.mtcw<<"\n";

                     //std::cout<<"Old Rotation Value"<<tex.mRcw<<"\n";
                     //tex.mRcw=(cv::Mat_<float>(3, 3) << received_R[0], received_R[1], received_R[2],received_R[3], received_R[4], received_R[5],received_R[6], received_R[7], received_R[8]);
                    //std::cout<<"New Rotation Value"<<tex.mRcw<<"\n";
...
//                        glBindTexture(GL_TEXTURE_2D, frameTex[indexCurr]);
    ...
    void ModelDrawer::settranslation(vector<float> M){
        //M.copyTo(received_M);
        received_M[0]=M[0];
        received_M[1]=M[1];
        received_M[2]=M[2];
    ...
    void ModelDrawer::setrotation(vector<float> R){
        received_R=R;
    }
